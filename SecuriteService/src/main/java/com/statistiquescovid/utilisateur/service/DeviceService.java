...
import com.statistiquescovid.utilisateur.repository.DeviceMetadataRepository;
...
public class DeviceService {
	...
	public void verifyDevice(CompteUtilisateur compteUtilisateur, HttpServletRequest request) throws IOException, GeoIp2Exception {
		...
		DeviceMetadata existingDevice = findExistingDevice(compteUtilisateur.getId(), deviceDetails, location);

		if (Objects.isNull(existingDevice)) {
			unknownDeviceNotification(deviceDetails, location, ip, compteUtilisateur.getEmail(), request.getLocale());

			DeviceMetadata deviceMetadata = new DeviceMetadata();
			deviceMetadata.setUserId(compteUtilisateur.getId());
